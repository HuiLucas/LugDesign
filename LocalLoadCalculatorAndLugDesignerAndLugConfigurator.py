...
def choose_kby(t, D, e):
    ...
    return kby
M_S =0 # 1.25 * 1.1 - 1   The safety factors are already applied at the locations where the material properties are used (x1.1) and for the forces (x1.25)
    ...
    for material in Material_List:
        ...
        for i in range(10, 500, i_step):
            ...
            for j in range(1, 50, j_step):
                t = j * 10 ** (-3)
                print(f"Progress: {round((i/500+j/(50*j_step))*100/1.062,1)}% of Material: {material}", flush=True)
