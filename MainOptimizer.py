...
if initial_design.N_lugs ==1:
    initial_design.D2_list = [2.2,2.2,2.2,2.2]
# ----------------------------------------------------------------------------------------------------------------------
...
for designindex in range(len(design_array)):
    ...
    if out1.h > out1.length and out1.N_Flanges == 2:
        changex = out1.h - out1.length
        out1.length = out1.h
        for j in range(len(out1.hole_coordinate_list)):
            deltaX = changex*0.5
            out1.hole_coordinate_list[j] = (out1.hole_coordinate_list[j][0] + deltaX, out1.hole_coordinate_list[j][1])
